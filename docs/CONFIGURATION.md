...
This document explains how to use each role provided in the Ansible collection `fujitsu.primergy`,
along with examples of playbooks.  
This configuration guide uses Ansible to perform various settings of PRIMERGY's iRMC,
install the OS (Windows Server), and configure the Windows Server.

## 2. PRIMERGY

Configure the iRMC (remote management feature) of PRIMERGY.

### Environment and setting sheet –Hardware-

**Note**:
The "Environment and setting sheet –Hardware-" is only available in Japanese.

Please also refer to the "Environment and setting sheet –Hardware-" provided for each PRIMERGY model
(hereinafter referred to as "Environment and setting sheet (PRIMERGY)").  
You can obtain the "Environment and setting sheet (PRIMERGY)" using the following steps:
...
please also refer to the "iRMC S6 Web Interface User Guide" (hereinafter referred to as "iRMC User Guide").  
You can obtain the "iRMC User Guide" using the following steps:
...
- Also refer to the
  ...
  in the [iRMC User Guide](#irmc-user-guide).
- For details on parameters, etc.,
...
- Also refer to the "2.4.5 User Management"
  in the [iRMC User Guide](#irmc-user-guide).
- For details on parameters, etc.,
...
- Executing this role performs a series of operations
  including inserting the installation media into the virtual drive,
  changing the boot priority to CD/DVD-ROM,
  and powering on the device.
- Since there is no built-in mechanism for automated installation,
  after powering on the device,
  you need to open the iRMC's AVR and operate the installer.
- AVR (Advanced Video Redirection) is the remote console function provided by iRMC.
  You can log in to iRMC (`https://{iRMC device IP address}/login`) using a web browser
  and open it from the icon in the upper right corner (tooltip "Launch AVR").
- After installation,
  when executing Ansible tasks targeting the Windows Server as the target node,
  you need to wait for the playbook execution until the Windows Server becomes usable as an Ansible target node.  
  Refer to the example `"ansible.builtin.wait_for_connection"` below
...
**Note**:
The "Environment and setting sheet –ServerView Installation Manager-" is only available in Japanese.

Please also refer to the sheet "Win2K**_Guide"
in the "Environment and setting sheet –ServerView Installation Manager-"
(hereinafter referred to as "Environment and setting sheet (Windows Server)").  
Although the various roles provided in this project have only been verified on Windows Server 2022,
this document is based on the "Win2K19" (Windows Server 2019) guide
in the "環境設定シート –ServerView Installation Manager編-" (CA92344-0149-07).
...
in the [Environment and setting sheet (Windows Server)](#environment-and-setting-sheet-serverview-installation-manager-).
...
- Depending on the language you set,
...
- Specify the path to the installer and `openjdk_installer` on the Ansible control node's file system.  
  Specify an absolute path or a relative path from the playbook.
...
7. Extract the `FTS_ServerViewManagementandServiceabilityDVD_*.zip` file and retrieve the ISO file
...
- For the `path` parameter, you can directly specify DSNAP (`*.exe`).  
  Although it increases the effort to extract DSNAP from the ISO file,
  it reduces the time required to transfer files to the target node, thereby reducing the playbook execution time.  
  To extract DSNAP from the ISO file,
  (on Windows) right-click the ISO file > Open with > Explorer to mount the ISO file,
